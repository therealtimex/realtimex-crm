...
export class SDKService {
            ...
            try {
                this.instance = new RealtimeXSDK({
                    realtimex: {
                        ...
                        apiKey: process.env.REALTTIMEX_API_KEY || 'SXKX93J-QSWMB04-K9E0GRE-J5DA8J0'
                    },
                    permissions: [
                ...
                this.instance.ping()
                    .then(() => console.log('[SDKService] Connected to RealTimeX Desktop'))
                    .catch((err) => console.warn('[SDKService] Desktop App not found:', err.message));
     ...
     */
    static async withTimeout(promise, timeoutMs = 30000, errorMsg = 'SDK request timed out') {
        let timeoutHandle;
    ...
    static async resolveChatProvider(settings = {}) {
        ...
        try {
            ...
            const preferred = providers.find(p => p.provider === this.DEFAULT_LLM_PROVIDER);

            if (preferred && preferred.models?.length > 0) {
                const model = preferred.models.find(m => m.id === this.DEFAULT_LLM_MODEL) || preferred.models[0];
                this.defaultChatProvider = { provider: preferred.provider, model: model.id };
            } else if (providers.length > 0 && providers[0].models?.length > 0) {
                this.defaultChatProvider = { provider: providers[0].provider, model: providers[0].models[0].id };
            }

            return this.defaultChatProvider || { provider: this.DEFAULT_LLM_PROVIDER, model: this.DEFAULT_LLM_MODEL };
        } catch (err) {
    ...
    static async resolveEmbedProvider(settings = {}) {
        ...
        try {
            ...
            if (preferred && preferred.models?.length > 0) {
                const model = preferred.models.find(m => m.id === this.DEFAULT_EMBED_MODEL) || preferred.models[0];
                this.defaultEmbedProvider = { provider: preferred.provider, model: model.id };
            } else if (providers.length > 0 && providers[0].models?.length > 0) {
                this.defaultEmbedProvider = { provider: providers[0].provider, model: providers[0].models[0].id };
            }

            return this.defaultEmbedProvider || { provider: this.DEFAULT_EMBED_PROVIDER, model: this.DEFAULT_EMBED_MODEL };
        } catch (err) {
    ...
    static async transcribeAudio(audioBuffer, settings = {}) {
        ...
        return this.withTimeout(
            sdk.audio.transcribe(audioBuffer, { ...settings })
        );
    }
    ...
    static async getEmbedProviders() {
        ...
        this.embedProvidersPromise = (async () => {
            try {
                ...
                console.warn('[SDKService] Failed to get embed providers:', err.message);
                return { success: false, providers: [] };
            } finally {
