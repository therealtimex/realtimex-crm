...
const NewMessageContext = React.createContext<React.MutableRefObject<
  string | null
> | null>(null);
...
const AssistantMessage = () => {
  ...
  const isNewMessage = newMessageContentRef
    ? textContent === newMessageContentRef.current
    : false;
...
const AIPageContent = () => {
  ...
  const fetchThreads = useCallback(async () => {
    try {
      ...
      const formattedThreads = (data as ThreadWithMessages[]).map((t) => ({
        ...
        last_message: t.chat_messages?.sort(
          (a, b) =>
      ...
      setActiveThreadId((prev) => {
        if (prev && formattedThreads.some((thread) => thread.id === prev))
          return prev;
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("realtimex-refresh-threads", handleRefresh);
  }, [fetchThreads]);
      ...
      run: async function* ({ messages, abortSignal }) {
        ...
        if (
          ...
          (currentThread.title === "New Conversation" ||
            currentThread.title === "Untitled Conversation")
        ) {
          updatedTitle =
      ...
      <div className="flex flex-col items-center justify-center h-[calc(100vh-120px)] text-center p-8 space-y-6 bg-muted/10 border-2 border-dashed rounded-3xl mx-4 my-2">
        ...
        <div className="space-y-2 max-w-sm">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            To use AI features, please ensure the RealTimeX Desktop app is
                ...
                <div className="flex items-center gap-3 min-h-11">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-tighter">
                      Powered by {settings.llm_provider || "Local LLM"} •{" "}
                      {settings.llm_model}
                    </p>
                  </div>
                </div>
                    ...
                    <ThreadPrimitive.Empty>
                      <div className="flex flex-col items-center justify-center h-full text-center space-y-6 max-w-md mx-auto py-12">
                        ...
                        <div className="space-y-2">
                          ...
                          <p className="text-sm text-muted-foreground leading-relaxed">
                            Your private, secure CRM assistant. I can help you
                            analyze data, draft emails, or summarize your day.
                          </p>
                        </div>
