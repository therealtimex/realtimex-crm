...
import { Terminal, ChevronDown, ChevronUp, AlertCircle, Info, Brain, Activity } from 'lucide-react';
...
export const LiveTerminal = ({ activityId }: { activityId?: string }) => {
    ...
    useEffect(() => {
        ...
        const channel = supabase
            ...
            .on(
                ...
                (payload) => {
                    setEvents((prev) => [...prev, payload.new as ProcessingEvent].slice(-100));
                }
            )
            .subscribe();
    ...
    const getEventIcon = (type: string) => {
        switch (type) {
            ...
            case 'acting': return <Activity className="h-3 w-3 text-blue-400" />;
            case 'analyzing': return <Info className="h-3 w-3 text-blue-400" />;
            case 'success': return <Activity className="h-3 w-3 text-green-400" />;
        ...
        <Card className={cn(
            ...
            >
                <div className="flex items-center space-x-2">
                    ...
                    <span className="text-xs font-mono font-bold uppercase tracking-wider">AI Live Terminal</span>
                    {events.length > 0 && (
            ...
            {isExpanded && (
                ...
                >
                    {events.length === 0 ? (
                        <div className="text-slate-600 italic">Waiting for AI events...</div>
                    ) : (
                        events.map((event) => (
                            <div key={event.id} className="flex space-x-2 animate-in fade-in slide-in-from-left-2 duration-300">
                                ...
                                <span className={cn(
                                    ...
                                    event.event_type === 'error' ? "text-red-400" :
                                        event.event_type === 'success' ? "text-emerald-400" : "text-slate-300"
                                )}>
                                    {event.content}
                                </span>
                            </div>
