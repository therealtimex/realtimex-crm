    ...
    const generateBriefing = async () => {
        ...
        try {
            ...
            const summaryResponse = await axios.post('/api/sdk/chat', {
                messages: [
                    { role: 'system', content: 'You are a CRM personal assistant. Summarize the user\'s agenda into a friendly, professional daily briefing (approx 2 sentences).' },
                    { role: 'user', content: `Tasks for today: ${JSON.stringify(tasks?.map(t => t.text))}` }
                ]
            });

            if (!summaryResponse.data.success) throw new Error("Briefing generation failed");

            const text = summaryResponse.data.content;

            // 2. Convert to speech via SDK TTS
            const ttsResponse = await axios.post('/api/sdk/tts', { text }, { responseType: 'blob' });
